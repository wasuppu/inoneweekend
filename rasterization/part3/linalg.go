...
func (v Vec[T]) Add(o Vec[T]) Vec[T] {
	asserrt(len(v) == len(o), "length of vector should be same")
	u := v.Clone()
...
func asserrt(cond bool, msg string) {
	if !cond {
		panic(msg)
	}
}
...
// Because the matrix is implemented by slices, it cannot be directly copied and requires the clone method
