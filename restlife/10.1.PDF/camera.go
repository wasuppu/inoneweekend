...
func (c *Camera) Render(world Hittable) {
	...
	WritePng("3-10.1", framebuffer, c.imageWidth, c.imageHeight)
}
...
func (c Camera) RayColor(r Ray, depth int, world Hittable) RGB {
	...
	surfacePdf := NewCosinePDF(rec.normal)
	scattered := Ray{rec.p, surfacePdf.Generate(), r.tm}
	pdfValue := surfacePdf.Value(scattered.dir)
