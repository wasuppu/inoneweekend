...
type Material interface {
	Scatter(in Ray, rec HitRecord) (bool, RGB, Ray, float64)
...
func (m EmptyMaterial) Scatter(in Ray, rec HitRecord) (bool, RGB, Ray, float64) {
	return false, RGB{}, Ray{}, 0
}
...
func (m Lambertian) Scatter(in Ray, rec HitRecord) (bool, RGB, Ray, float64) {
	uvw := NewONB(rec.normal)
	scatterDirection := uvw.Transform(RandomCosineDirection())

	scattered := Ray{rec.p, scatterDirection.Normalize(), in.tm}
	attenuation := m.tex.Value(rec.u, rec.v, rec.p)
	pdf := uvw.W().Dot(scattered.dir) / math.Pi
	return true, attenuation, scattered, pdf
}
...
func (m Metal) Scatter(in Ray, rec HitRecord) (bool, RGB, Ray, float64) {
	...
	return scattered.dir.Dot(rec.normal) > 0, attenuation, scattered, 0
}
...
func (m Dielectric) Scatter(in Ray, rec HitRecord) (bool, RGB, Ray, float64) {
	...
	return true, attenuation, scattered, 0
}
...
func (m DiffuseLight) Scatter(in Ray, rec HitRecord) (bool, RGB, Ray, float64) {
	return false, RGB{}, Ray{}, 0
}
...
func (m Isotropic) Scatter(in Ray, rec HitRecord) (bool, RGB, Ray, float64) {
	attenuation := m.tex.Value(rec.u, rec.v, rec.p)
