...
func (c *Camera) Render(world Hittable) {
	...
	WritePng("3-9.2", framebuffer, c.imageWidth, c.imageHeight)
}
...
func (c Camera) RayColor(r Ray, depth int, world Hittable) RGB {
	...
	onLight := Point3{RandomRange(213, 343), 554, RandomRange(227, 332)}
	toLight := onLight.Sub(rec.p)
	distanceSquared := toLight.Dot(toLight)
	...
	pdfValue := distanceSquared / (lightCosine * float64(lightArea))
	scattered := Ray{rec.p, toLight, r.tm}

	scatteringPdf := rec.mat.ScatteringPdf(r, rec, scattered)

	colorFromScatter := attenuation.Muln(scatteringPdf).Mul(c.RayColor(scattered, depth-1, world)).Divn(pdfValue)
