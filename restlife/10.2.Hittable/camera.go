...
func (c *Camera) Render(world Hittable, lights Hittable) {
	...
	WritePng("3-10.2", framebuffer, c.imageWidth, c.imageHeight)
}
...
func (c Camera) RayColor(r Ray, depth int, world Hittable, lights Hittable) RGB {
	...
	lightPdf := HittablePDF{lights, rec.p}
	scattered := Ray{rec.p, lightPdf.Generate(), r.tm}
	pdfValue := lightPdf.Value(scattered.dir)
